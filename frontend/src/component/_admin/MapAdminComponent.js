...
export default class MapAdminComponent extends Component {
    constructor(props) {
        super(props);
        this.state = {
        ...
        managePostHelper.getall().then(data=>{
            ...
            if (data.status) {
                this.setState({
                    ...
                    marker: data.data.map(c => { return { address: c.DiaChi, image: c.avatar, price: c.price, local: [c.lat, c.lng], id: c.id, title: c.title ,status:c.status} })
                })
            }
    ...
    render() {
        return (
            ...
            {this.state.marker.map((positon,index,array) => <Marker
               ...
               icon={!positon.status? duckIcon:marker}
                    ...
                    <Link style={{textDecoration:"none"}} to={"/chitiet/" + positon.id} className="checkup_header">
                        <div className="row">
                            <div className="col-md-6">
                                ...
                                <strong style={!positon.status ?{color:"red"}:{}} className="address">
                                        {positon.status?"Đã duyệt":"Chưa duyệt"}
                                    </strong>
                            </div>
                            <div className="col-md-6">
                                <img width={150}
